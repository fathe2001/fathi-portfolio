...
describe('Experience', () => {
  ...
  it('renders entries in reverse-chronological order, not array order (Req 4.2)', () => {
    ...
    const dyIndex = listItems.findIndex((item) =>
      item.textContent?.includes('Dynamic Yield'),
    );
    const atidimIndex = listItems.findIndex((item) =>
      item.textContent?.includes('Atidim Program'),
    );
  ...
  it('renders achievement bullets as a list (Req 4.4)', () => {
    render(<Experience entries={[recentEntry]} />);
    expect(
      screen.getByText('Maintained production infrastructure'),
    ).toBeInTheDocument();
